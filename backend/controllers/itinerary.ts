...
import { createItinerary, deleteItinerary, getCollabItineraries, getCreatedItineraries, getItineraries, getItineraryById, updateItinerary } from "../services/itinerary";
...
// @access  Public
export const getCreatedItinerariesPublicApi = async (req: Request, res: Response) => {
    try {
        const page = parseInt(req.query.page as string) || 1;
        const limit = parseInt(req.query.limit as string) || 10;    
        const ownerId = parseInt(req.params.ownerId);
        const itineraries = await getCreatedItineraries(ownerId, false, page, limit) || {};
        res.status(200).json(itineraries);
    } catch (error: any) {
        res.status(500).json({ message: error.message || "Internal Server Error" });
    }
...
export const createItineraryApi = async (req: Request, res: Response) => {
    try {
        ...
        if (!validatedFields.success) {
            ...
            const { title, location, visibility, start_date, end_date, collaborators } = validatedFields.data;
            let itinerary = await createItinerary(ownerId, title, location, visibility, start_date, end_date, collaborators);
...
// @access  Protected
export const getCreatedItinerariesProtectedApi = async (req: Request, res: Response) => {
    try {
        const page = parseInt(req.query.page as string) || 1;
        const limit = parseInt(req.query.limit as string) || 10;
        const ownerId = parseInt(req.params.ownerId);
        ...
        let itineraries = await getCreatedItineraries(ownerId, ownerId == userId, page, limit) || {};
        res.status(200).json(itineraries);
    } catch (error: any) {
        res.status(500).json({ message: error.message || "Internal Server Error" });
    }
...
export const updateItineraryApi = async (req: Request, res: Response) => {
    try {
        const validatedFields = await updateItinerarySchema.safeParseAsync(req.body);
        if (!validatedFields.success) {
            res.status(400).json({
                message: validatedFields.error.errors
                  .map((err) => `${err.path.join(".")}: ${err.message}`)
