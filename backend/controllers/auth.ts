...
import { generateConfirmAccountToken, getConfirmAccountTokenByToken } from '../services/token';
...
export const registerUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!validatedFields.success) {
            errorFlag = true;
            errorMessage = validatedFields.error.errors.map(err => `${err.path.join('.')}: ${err.message}`).join(', ');
        } else {
            ...
            if (existingUser && !existingUser.email_verified) {
                ...
                errorFlag = true;
                errorMessage = "Account has not been verified! A new confirmation email has been sent!";
        ...
        if (!validatedFields.success) {
            ...
            if (userFromDB) {
                ...
                if (!isMatch) {
                    ...
                    token = jwt.sign({ id: userFromDB.id }, process.env.JWT_SECRET as string, {
                        expiresIn: '7d',
                    });
                    user = {
                        id: userFromDB.id,
...
export const confirmAccount = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!existingToken) {
            ...
            const hasExpired = new Date(existingToken.created_at).getTime() + tokenExpirationPeriod < Date.now();
