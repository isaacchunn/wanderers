...
export default async function HomePage() {
    const ownerId = 1;
    const userItinerary: Itinerary[] = (await fetchUserItinerary(ownerId)) || [];
    console.log("User Itinerary:", userItinerary);
    const publicItinerary: Itinerary[] = (await fetchPublicItinerary()) || [];
    const CollabItinerary: Itinerary[] = (await fetchCollabItinerary()) || [];
            ...
            <div className="min-h-screen bg-background">
                <main className="container px-4 py-12">
                    ...
                    <div className="mt-12">
                        ...
                        >
                            <CarouselContent className="-ml-2 md:-ml-4">
                                {CollabItinerary.map((collabI) => (
                                    <CarouselItem
                                        key={collabI.id}
                                        className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3"
                                    >
                                        <Link href={`/itinerary/${collabI.id}`}>
                                            <Card className="overflow-hidden transition-colors hover:bg-muted/50">
                                                <CardContent className="p-0">
                                                    <div className="relative aspect-[4/3] w-full">
                                                        <Image
                                                    ...
                                                    <div className="space-y-3 p-4">
                                                        ...
                                                        <div className="space-y-2 text-sm text-muted-foreground">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <Calendar className="h-4 w-4" />
                                                                <span>
                                                                    {`${new Date(collabI.start_date).toLocaleDateString()} - ${new Date(collabI.end_date).toLocaleDateString()}`}
                                                                </span>
                                                            </div>
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <Clock className="h-4 w-4" />
                                                                <span>
                                                                    {`${Math.floor((getDate(collabI.end_date) - getDate(collabI.start_date)) / (1000 * 60 * 60 * 24)) + 1} Day(s)`}
                                                                </span>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <h2 className="line-clamp-1 text-m font-regular tracking-tight">
                                                                    Other Collaborators
                                                                </h2>
                                                                <Users2 className="h-4 w-4" />
                                                                <span>
